...
UI_LOGO_F=() UI_PHASE=0 UI_ANIM=0
# 0.3s 一帧。sbs 跑在服务器上，每一帧都要过 SSH 传到本地，菜单开着就一直在
# 发 —— 帧率减半等于每秒少发一半字节（6.8 -> 3.4 KB/s）
UI_ANIM_TICK=0.3
# 三条时间轴都由帧号推导，且在最后一帧同时归零 —— 不同时归零的话，循环接回
# 起点时会跳一下。0.3s 一帧，300 帧正好 90 秒，三组色带各占 30 秒。
#   色相  (f*80) % 2000   24000 能被 2000 整除
#   呼吸  (f*80) % 4000   也能被 4000 整除
#   色带  f*3000/300      走完回到第 0 组
#
# 相位步长保持 80、没跟着帧率一起加倍：色带扫一圈从 3.75s 变成 7.5s，每帧的
# 色差更小，看着反而更顺 —— 这个动画本来就该是慢慢漂的
UI_PH_STEP=80 UI_FRAMES=300 UI_LOGO_CUR=''
