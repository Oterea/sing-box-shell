...
ui_logo_render() {
    ...
    local r g b
    seg=$((UI_NSTOP - 1))
    # 呼吸：吸气段和呼气段各自把进度归一化到 0..1000，再过一遍 smoothstep
    # 缓动。原来是三角波 —— 匀速上匀速下、到顶到底硬拐弯，那个拐点是整段
    # 动画里最扎眼的地方。缓动之后两头斜率归零（峰值每帧只变 3‰），
    # 中段快（吸气 45‰、呼气 23‰），读起来是活的而不是机械往返。
    # bash 没有 cos，p²(3-2p) 是同形状的整数近似，与升余弦最大差 4‰
    br=$(((f + UI_BR_IN) % UI_BREATH))
    if [ "$br" -lt "$UI_BR_IN" ]; then
        br=$((br * 1000 / UI_BR_IN))
    else
        br=$(((UI_BREATH - br) * 1000 / (UI_BREATH - UI_BR_IN)))
    fi
    br=$((br * br * (3000 - 2 * br) / 1000000))
    br=$((UI_BR_LO + br * (1000 - UI_BR_LO) / 1000))
