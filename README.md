...
命令如果停在 `ubuntu@` 上没有地址，说明这台机器在 NAT 后面（云主机基本都是），
网卡上只有内网地址，它自己看不到公网 IP。粘贴之后直接把你 ssh 用的地址敲在
`@` 后面回车即可 —— 不留占位符就是为了省掉「先删再填」这一步。
本地侧统一 +10000，避开本机可能也在跑的 sing-box —— 官方客户端那份模板占的是同一批
端口号，不错位会撞 `bind: Address already in use`。面板地址里的 `port=` 参数会跟着一起错位。

隧道挂着的时候，本机浏览器打开命令下面打印的那两个地址即可（`/dashboard` 这个
路径是固定的，跟配置里 `dashboard.path` 无关 —— 那个是存文件的本地目录名）。
ctrl-c 关掉。
